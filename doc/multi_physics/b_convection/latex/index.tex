...
The equation that governs the evolution of the temperature field is the advection-\/diffusion equation where the \char`\"{}wind\char`\"{} is the fluid velocity. Thus, \begin{center}  \[ \frac{\partial \theta^{*}}{\partial t^{*}} +
...
\item Given the solution at time $ t = t_{\mbox{orig}} $ , the unsteady Newton solver {\ttfamily  Problem\+::unsteady\+\_\+newton\+\_\+solve(dt,...) } increments time by {\ttfamily dt}, shifts the \char`\"{}history\char`\"{} values and then computes the solution at the advanced time, $ t =
    t_{\mbox{orig}} $ + {\ttfamily dt}. On return from this function, all unknown {\ttfamily Data} values (and the corresponding \char`\"{}history\char`\"{} values) will have been assigned their correct values so that the solution at time $ t =
