...
\hline
\begin{center} {\bfseries{The two-\/dimensional linear wave equation in a rectangular domain.}} \end{center}  Solve  \[
\sum_{i=1}^2\frac{\partial^2 u}{\partial x_i^2}
= \frac{\partial^2 u}{\partial t^2} + f\left(x_1,x_2,t\right),
 \ \ \ \ \ \ \ \ \ \ (1)
\] in the rectangular domain $ D = 
\left\{ (x_1,x_2) \ \bigg| \ x_1 \in [0,1],\ x_2 \in [0,2] \right\} $, subject to the Dirichlet boundary conditions  \[
...
\ \ \ \ \ \ \ \ \ \ (4)
\] where the functions $ g_0, h_0$ and $ k_0 $ are given.   \\\cline{1-1}
\end{longtabu}
\end{center} 
...
\zeta=\cos\left(\phi\right)x_1+\sin\left(\phi\right)x_2,
 \ \ \ \ \ \ \ \ \ \ (6)
\] is the travelling-\/wave coordinate. The solution represents a tanh step profile that propagates with unit speed through the domain. The parameter $ \alpha $ controls the steepness of the step while $ \phi $ controls the orientation of the step in the $ (x_1,x_2) $ -\/ plane.
...
\DoxyCodeLine{\textcolor{comment}{///\ Namespace\ for\ exact\ solution\ for\ LinearWave\ equation\ }\textcolor{comment}{}}
...
\DoxyCodeLine{\textcolor{comment}{}\ \mbox{\hyperlink{classLinearWaveProblem}{LinearWaveProblem}}(\textcolor{keyword}{const}\ \textcolor{keywordtype}{unsigned}\&\ nx,\ \textcolor{keyword}{const}\ \textcolor{keywordtype}{unsigned}\&\ ny,\ }
...
In problems in which first time-\/derivatives are discretised by {\ttfamily BDF} timesteppers, the \char`\"{}history values\char`\"{} represent the values of the solution at previous timesteps, i.\+e. for a {\ttfamily Data} object that is associated with a timestepper from the {\ttfamily BDF} family, {\ttfamily Data\+::value(t,i)} returns the i-\/th value stored in the {\ttfamily Data} object, at the t-\/th previous timestep. We recall that when assigning initial conditions for computations with higher-\/order timesteppers from the {\ttfamily BDF} family, the number of (mathematical) initial conditions (the solution at the intial time, $ t=t_0 $ , say) is {\itshape smaller} than the number of \char`\"{}history values\char`\"{} required by the timestepper. For instance, a computation with a {\ttfamily BDF$<$2$>$} timestepper and a timestep {\ttfamily dt} not only requires the solution at $ t=t_0
$ but also the solution at $ t=t_0- {\tt dt} $ . In most of our examples, this did not cause any problems. We either started the simulation with an \char`\"{}impulsive start\char`\"{} by setting {\itshape all} history values to the solution at $ t=t_0 $ , or we assigned the history values from an explicitly-\/given function (usually an exact solution of the governing equations), in which case the evaluation of the history values at previous timesteps was trivial.
...
\begin{DoxyItemize}
\item t=0\+: The i-\/th value at at present time, {\ttfamily Time\+\_\+pt-\/\texorpdfstring{$>$}{>}time()} 
\item t=1\+: The i-\/th value at the previous timestep, {\ttfamily Time\+\_\+pt-\/\texorpdfstring{$>$}{>}time()-\/dt} 
\item ...
\item t=NSTEPS\+: The i-\/th value at the NSTEPS-\/th previous timestep, i.\+e. at {\ttfamily Time\+\_\+pt-\/\texorpdfstring{$>$}{>}time()-\/NSTEPS$\ast$dt} if the timestep dt was kept constant.
\item t=NSTEPS+1\+: The 1st time derivative (= \char`\"{}the velocity\char`\"{}) of the i-\/th value at the previous timestep, {\ttfamily Time\+\_\+pt-\/\texorpdfstring{$>$}{>}time()-\/dt} 
\item t=NSTEPS+2\+: The 2nd time derivative (= \char`\"{}the acceleration\char`\"{}) of the i-\/th value at the previous timestep, {\ttfamily Time\+\_\+pt-\/\texorpdfstring{$>$}{>}time()-\/dt}.
