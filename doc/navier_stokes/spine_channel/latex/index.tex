...
Further, assume that the mesh topology is such that the mesh\textquotesingle{}s $ N_{node} $ nodes are distributed along $ N_{spine} $ lines that are (topologically) orthogonal to the free boundary. We refer to these lines as the \char`\"{}spines\char`\"{} and denote the \char`\"{}height\char`\"{} of the domain, measured along spine $ s $ by $ H_s \ (s=1,...,N_{spine}) $. We associate each node with a particular spine (so that node $ j $ is located on spine $ s_j$) and locate it along a fixed fraction $ \omega_j $ along \char`\"{}its\char`\"{} spine. The position of node $ j $ may therefore be written as  \[
{\bf x}_j =  {\bf B}_{s_j} + \omega_j \, H_{s_j}  \, 
{\bf S}_{s_j} \ \ \ \ \ \ \ \ \ \ \ \ \  (1) 
\] where $ {\bf B}_s $ is the vector to the \char`\"{}base\char`\"{} of spine $ s $, and $ {\bf S}_s $ the unit vector along that spine.
