...
\] where $ u(x,y) $ is complex-\/valued. If the wavespeed is constant this transforms (1) into the \href{../../../helmholtz/scattering/html/index.html}{\texttt{ standard form of the Helmholtz equation}}  \[
...
\begin{DoxyItemize}
\item For layers that are aligned with the y axis (such as the left and right PML layers in the sketch) we set  \[
  \gamma_{x}(x)  = 
  ...
  \] where $ X_{PML} $ is the x-\/coordinate of the outer boundary of the PML layer, and  \[
   \gamma_y = 1.
  \]
\item For layers that are aligned with the x axis (such as the top and bottom PML layers in the sketch) we set  \[ 
  \gamma_x = 1,
  \] and  \[
  \gamma_{y}(y) = 
...
\DoxyCodeLine{\textcolor{comment}{///\ Problem\ class\ to\ demonstrate\ use\ of\ perfectly\ matched\ layers}\textcolor{comment}{}}
\DoxyCodeLine{\textcolor{comment}{///\ for\ Helmholtz\ problems.}}
